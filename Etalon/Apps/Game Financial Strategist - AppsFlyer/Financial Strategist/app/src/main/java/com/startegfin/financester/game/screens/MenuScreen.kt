...
class MenuScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private val nameFontMedium = "Medium"
    private val nameFontBold   = "Bold"

    private val fontMediumBold = Font().setFamily(Font.FontFamily(arrayOf(
        Font(fontMedium24).setName(nameFontMedium),
        Font(fontBold24).setName(nameFontBold),
    )))

    private val textUsed = "[@$nameFontMedium]Использовано: [@$nameFontBold]${Balance.balanceUsed.toBalance} ₽ [@$nameFontMedium]из[@$nameFontBold] ${Balance.balance.toBalance} ₽"
