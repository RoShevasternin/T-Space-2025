...
import com.startegfin.financester.game.actors.ADohodItemGroup
...
class AddDohodScreen(override val game: LibGDXGame) : AdvancedScreen() {
    ...
    private val itemsPanel   = ADohodItemGroup(this)
    ...
    private fun AdvancedGroup.addARoshod() {
        ...
        aRoshod.apply {
            ...
            setOnClickListener(game.soundUtil) {
                tmpGroup.animHide(TIME_ANIM) {
                    game.navigationManager.navigate(AddRoshodScreen::class.java.name)
                }
            }
        }
    }
        ...
        btnSave.apply {
            ...
            setOnClickListener {
                if (enteredSuma <= 0 || enteredCategoryIndex == -1) {
                    ...
                    btnSave.disable()
                    game.dataStoreDohodUtil.update { listDohod ->
                        listDohod.toMutableList().apply { add(
                            Dohod(
    ...
    private fun AdvancedStage.addItemsPanel() {
        addActor(itemsPanel)
        itemsPanel.apply {
            setBounds(0f,-350f,754f,311f)
            blockSelect = { selectedItemIndex ->
                enteredCategoryIndex = selectedItemIndex
                imgCategory.drawable = TextureRegionDrawable(game.all.listDohodIcon[selectedItemIndex])
                lblCategory.setText(game.listCategoryDohod[selectedItemIndex])
                hideItemsPanel()
            }
        }
    }
    ...
    private fun hideItemsPanel() {
        itemsPanel.addAction(Actions.sequence(
            Actions.moveTo(0f, -350f, TIME_ANIM, Interpolation.sineIn),
