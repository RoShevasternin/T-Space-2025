package com.progressmas.samsuster.game.screens

import com.progressmas.samsuster.R
import com.progressmas.samsuster.game.LibGDXGame
import com.progressmas.samsuster.game.actors.AButton
import com.progressmas.samsuster.game.actors.AClosedLevel
import com.progressmas.samsuster.game.actors.panel.*
import com.progressmas.samsuster.game.utils.*
...
class Work_3_Screen(override val game: LibGDXGame) : AbstractWorkScreen(ScreenType._3) {
    ...
    override val panelImprovements = AImprovements_3_Panel(this, screenType)
    override val panelInvestments  = AInvestments_3_Panel(this, screenType)
    ...
    override fun show() {
        ...
        game.backgroundColor = GColor.lvl_3
        game.activity.setNavBarColor(R.color.lvl_3)
    ...
    override fun AdvancedGroup.addActorsOnMainGroup() {
        ...
        if (game.dataStore.level_3.not()) {
            stageUI.root.disable()

            topStageUI.addAndFillActor(
                AClosedLevel(this@Work_3_Screen, screenType.ordinal).also { level ->
                    level.blockOpenLvl = { price ->
                        if (price <= game.dataStore.balance) {
                            ...
                            game.dataStore.updateLVL_3 { true }
                            level.close()
    ...
    private fun AdvancedGroup.addBtns() {
        ...
        btnLeft.setOnClickListener {
            animHide(TIME_ANIM) {
                game.navigationManager.navigate(Work_2_Screen::class.java.name)
            }
        }
        btnRight.setOnClickListener {
            animHide(TIME_ANIM) {
                game.navigationManager.navigate(Work_4_Screen::class.java.name, Work_3_Screen::class.java.name)
            }
        }
    }
