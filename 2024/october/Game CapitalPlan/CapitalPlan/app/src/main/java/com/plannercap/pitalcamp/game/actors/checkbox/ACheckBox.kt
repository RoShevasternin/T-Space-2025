package com.plannercap.pitalcamp.game.actors.checkbox
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Type) = when(type) {
        Type.MAIN -> ACheckBoxStyle(
            default = screen.game.all.main_press,
            checked = screen.game.all.main_def,
        )
        Type.BLOG -> ACheckBoxStyle(
            default = screen.game.all.blog_press,
            checked = screen.game.all.blog_def,
        )
        Type.INC_EXP -> ACheckBoxStyle(
            default = screen.game.all.box_expense,
            checked = screen.game.all.box_income,
        )
    }
