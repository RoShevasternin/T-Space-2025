...
import com.ekobioznaher.sugertogerra.game.utils.TextureEmpty
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchUp(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int) {
            if (isWithin) {
                if (isSound) gdxGame.soundUtil.apply { play(click) }
    ...
    fun getStyleByType(type: Type) = when(type) {
        Type.POINT -> ACheckBoxStyle(
            default = gdxGame.assetsAll.gray,
            checked = gdxGame.assetsAll.green,
        )
        Type.FRAME -> ACheckBoxStyle(
            default = TextureEmpty.region,
            checked = gdxGame.assetsAll.frame,
        )
        Type.ANSWER -> ACheckBoxStyle(
            default = gdxGame.assetsAll.ans_def,
            checked = gdxGame.assetsAll.ans_check,
        )
