package com.ayrym.inperdader.game.actors.checkbox
...
open class ACheckBox(
    ...
    open fun getListener() = object : InputListener() {
        ...
        override fun touchUp(event: InputEvent?, x: Float, y: Float, pointer: Int, button: Int) {
            ...
            log("dd = $touchPointDown | $touchPointUp")
            if (isWithin) {
    ...
    fun getStyleByType(type: Type) = when(type) {
        Type.CIRCLE -> ACheckBoxStyle(
            default = gdxGame.assetsAll.circle_def,
            checked = gdxGame.assetsAll.circle_check,
        )
    }
