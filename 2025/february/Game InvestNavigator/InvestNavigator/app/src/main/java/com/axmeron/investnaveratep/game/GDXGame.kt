package com.axmeron.investnaveratep.game
...
import com.axmeron.investnaveratep.MainActivity
import com.axmeron.investnaveratep.appContext
import com.axmeron.investnaveratep.game.manager.MusicManager
import com.axmeron.investnaveratep.game.manager.NavigationManager
...
import com.axmeron.investnaveratep.game.manager.util.MusicUtil
import com.axmeron.investnaveratep.game.manager.util.SoundUtil
import com.axmeron.investnaveratep.game.manager.util.SpriteUtil
import com.axmeron.investnaveratep.game.screens.LoaderScreen
import com.axmeron.investnaveratep.game.utils.GameColor
import com.axmeron.investnaveratep.game.utils.advanced.AdvancedGame
import com.axmeron.investnaveratep.game.utils.disposeAll
import com.axmeron.investnaveratep.util.Gist
...
class GDXGame(val activity: MainActivity) : AdvancedGame() {
    ...
    private fun track() {
        ...
        val track = sharedPreferences.getString("babret", "smak") ?: "smak"
        ...
        try {
            if (track == "smak") {
                coroutine.launch(Dispatchers.Main) {
                    ...
                    if (simJSON != null) {
                        AppsFlyerLib.getInstance().init(simJSON.bob, getAppsFlyerConversionListener(simJSON.lol), appContext)
                        AppsFlyerLib.getInstance().start(activity, simJSON.bob, getAppsFlyerRequestListener())
                    } else {
                        GLOBAL_isGame = true
                    }
                }
    ...
    private fun getAppsFlyerConversionListener(liber: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val link = "$liber?rbxpjc=$campaign&zdljjz=$afAd&kkbag=$media&duzcz=$afId"
                log("link = $link")

                coroutine.launch(Dispatchers.IO) { sharedPreferences.edit().putString("babret", link).apply() }
