...
class AMainCalculatorLeasing(
    override val screen: CalculatorLeasingScreen,
): AdvancedMainGroup() {
    ...
    private val lblTitle     = Label("Калькулятор Лизинга", ls43)
    ...
    private fun AVerticalGroup.addItems() {
        val listDataItem = listOf(
            ...
            DataItem("", "Период (месяцы)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) },
            DataItem("", "Стоимость объекта", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(8).toSeparateWithSymbol(' ') },
            DataItem("", "Лизинговый процент (%)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) + "%" },
            DataItem("", "Первичный взнос", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(8).toSeparateWithSymbol(' ') },
            DataItem("", "Процент комиссии (%)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) + "%" },
            DataItem("", "Месячная амортизация (%)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) + "%" },
        ).onEach { data ->
        ...
        btnCalculate.setOnClickListener {
            if (checkListDataItem(listDataItem)) {
                ...
                gdxGame.ds_DataItems.update { list ->
                    ...
                    val dataItems = DataItems(DataItemType.Leasing, listOf(
                        name,
                        ...
                        commission.toInt().toString().toSeparateWithSymbol(' ') + " ₽",
                        remainingCost.toInt().toString().toSeparateWithSymbol(' ') + " ₽",
                    ))
                    mList.add(dataItems)
                    mList
                }
