...
class AMainCalculatorDeposit(
    override val screen: CalculatorDepositScreen,
): AdvancedMainGroup() {
    ...
    private val lblTitle     = Label("Калькулятор Депозита", ls43)
    ...
    private fun AVerticalGroup.addItems() {
        val listDataItem = listOf(
            ...
            DataItem("", "Срок (Месяцы)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) },
            DataItem("", "Сумма вклада (₽)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(8).toSeparateWithSymbol(' ') },
            DataItem("", "Пополнение вклада в месяц (₽)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(8).toSeparateWithSymbol(' ') },
            DataItem("", "Процент (%)", Input.OnscreenKeyboardType.NumberPad) { text -> text.take(2) + "%" },
        ).onEach { data ->
        ...
        btnCalculate.setOnClickListener {
            if (checkListDataItem(listDataItem)) {
                ...
                val months                : Int     = listDataItem[1].resultText.toInt()    // Термін вкладу (у місяцях)
                val initialDeposit        : Double  = listDataItem[2].resultText.toDouble() // Початкова сума вкладу (₽)
                val monthlyAddition       : Double  = listDataItem[3].resultText.toDouble() // Щомісячне поповнення (₽)
                val annualInterestRate    : Double  = listDataItem[4].resultText.toDouble() // Річна процентна ставка (%)

                val monthlyInterestRate = (annualInterestRate / 100) / 12

                var totalDeposit = initialDeposit
                var totalInterest = 0.0

                repeat(months) {
                    val interest = totalDeposit * monthlyInterestRate
                    totalInterest += interest
                    totalDeposit += interest + monthlyAddition
                }

                gdxGame.ds_DataItems.update { list ->
                    val mList = list.toMutableList()
                    val dataItems = DataItems(DataItemType.Deposit, listOf(
                        name,
                        totalInterest.toInt().toString().toSeparateWithSymbol(' ') + " ₽", // "Сума процента"
                        totalDeposit.toInt().toString().toSeparateWithSymbol(' ') + " ₽",  // "Основна сума вкладу"
                    ))
                    mList.add(dataItems)
                    mList
                }
