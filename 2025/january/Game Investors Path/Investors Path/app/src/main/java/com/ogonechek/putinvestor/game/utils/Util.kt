...
import com.ogonechek.putinvestor.game.GDXGame
...
fun getMonthWord(count: Int): String {
    val lastTwoDigits = count % 100
    val lastDigit = count % 10

    return when {
        lastTwoDigits in 11..19 -> "Месяцев" // Всі числа від 11 до 19 закінчуються на "Месяцев"
        lastDigit == 1 -> "Месяц" // Число закінчується на 1
        lastDigit in 2..4 -> "Месяца" // Числа закінчуються на 2, 3, 4
        else -> "Месяцев" // Інші випадки
    }
}

fun addMonthsToDate(months: Int): String {
    val calendar = Calendar.getInstance() // Поточна дата
    calendar.add(Calendar.MONTH, months) // Додаємо місяці
