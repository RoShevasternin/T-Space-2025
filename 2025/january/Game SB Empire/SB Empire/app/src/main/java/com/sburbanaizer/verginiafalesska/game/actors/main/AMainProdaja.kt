...
import com.sburbanaizer.verginiafalesska.game.actors.APlusGold
...
class AMainProdaja(
    override val screen: ProdajaScreen,
): AdvancedGroup() {

    private val levelIndex = gdxGame.ds_Level.flow.value.lvlProdaja - 1

    private val imgMap     = Image(gdxGame.assetsAll.listProdaja[levelIndex])
    ...
    private val btnGo      = AButton(screen, AButton.Type.GOtoDO)
    ...
    private fun addBtns() {
        val cost = when(gdxGame.ds_Level.flow.value.lvlProdaja) {
            1 -> 10
            2 -> 15
            3 -> 20
            4 -> 25
        ...
        btnDob.apply {
            ...
            setOnClickListener(gdxGame.soundUtil.gold) {
                if (gdxGame.ds_Maslo.flow.value - cost >= 0) {
    ...
    private fun addAPlusGold() {
        addActor(aPlusGold)
        aPlusGold.setBounds(16f, 487f, 682f, 179f)
    }
