package com.padrestoranom.easypaydonalds.game.actors.button
...
import com.padrestoranom.easypaydonalds.game.utils.TextureEmpty
...
open class AButton(
    ...
    fun setOnClickListener(sound: SoundUtil.AdvancedSound? = null, block: () -> Unit) {
        this.sound   = sound
        onClickBlock = block
    }
    ...
    private fun getStyleByType(type: Type) = when(type) {
        ...
        Type.Blue -> AButtonStyle(
            default  = TextureRegionDrawable(gdxGame.assetsLoader.btn),
            pressed  = TextureRegionDrawable(gdxGame.assetsLoader.btn_press),
            disabled = TextureRegionDrawable(gdxGame.assetsLoader.btn_press),
        )
    }
