package com.borubashka.arsemajeg.game
...
import com.borubashka.arsemajeg.MainActivity
import com.borubashka.arsemajeg.appContext
import com.borubashka.arsemajeg.game.dataStore.DS_LevelData
import com.borubashka.arsemajeg.game.dataStore.DS_Score
import com.borubashka.arsemajeg.game.manager.MusicManager
import com.borubashka.arsemajeg.game.manager.NavigationManager
import com.borubashka.arsemajeg.game.manager.SoundManager
import com.borubashka.arsemajeg.game.manager.SpriteManager
import com.borubashka.arsemajeg.game.manager.util.MusicUtil
import com.borubashka.arsemajeg.game.manager.util.SoundUtil
import com.borubashka.arsemajeg.game.manager.util.SpriteUtil
import com.borubashka.arsemajeg.game.screens.LoaderScreen
import com.borubashka.arsemajeg.game.utils.GameColor
import com.borubashka.arsemajeg.game.utils.advanced.AdvancedGame
import com.borubashka.arsemajeg.game.utils.disposeAll
import com.borubashka.arsemajeg.game.utils.gdxGame
import com.borubashka.arsemajeg.util.Gist
...
class GDXGame(val activity: MainActivity) : AdvancedGame() {
    ...
    val sharedPreferences: SharedPreferences = appContext.getSharedPreferences("Ruble", MODE_PRIVATE)

    val ds_ItemData = DS_LevelData(coroutine)
    val ds_Score    = DS_Score(coroutine)
    ...
    private fun pardonMyCupidon() {
        ...
        val path = sharedPreferences.getString("Buble", "Nokia") ?: "Nokia"
        ...
        try {
            if (path == "Nokia") {
                coroutine.launch(Dispatchers.Main) {
                    ...
                    if (poket != null) {
                        AppsFlyerLib.getInstance().init(poket.kaybabra, getAppsFlyerConversionListener(poket.labra), appContext)
                        AppsFlyerLib.getInstance().start(gdxGame.activity, poket.kaybabra, getAppsFlyerRequestListener())
                    } else {
                        GDX_GLOBAL_isGame = true
                    }
                }
    ...
    private fun getAppsFlyerConversionListener(sma: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val link = "$sma?txpkq=$campaign&kvwfmtwa=$afAd&uyfwcqf=$media&zlviibppb=$afId"
                log("link = $link")

                coroutine.launch(Dispatchers.IO) { sharedPreferences.edit().putString("Buble", link).apply() }
