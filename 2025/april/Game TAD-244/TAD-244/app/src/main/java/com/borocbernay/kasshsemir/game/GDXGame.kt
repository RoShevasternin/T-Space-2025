package com.borocbernay.kasshsemir.game
...
import com.borocbernay.kasshsemir.MainActivity
import com.borocbernay.kasshsemir.appContext
import com.borocbernay.kasshsemir.game.dataStore.DS_ItemData
import com.borocbernay.kasshsemir.game.manager.MusicManager
import com.borocbernay.kasshsemir.game.manager.NavigationManager
import com.borocbernay.kasshsemir.game.manager.SoundManager
import com.borocbernay.kasshsemir.game.manager.SpriteManager
import com.borocbernay.kasshsemir.game.manager.util.MusicUtil
import com.borocbernay.kasshsemir.game.manager.util.SoundUtil
import com.borocbernay.kasshsemir.game.manager.util.SpriteUtil
import com.borocbernay.kasshsemir.game.screens.LoaderScreen
import com.borocbernay.kasshsemir.game.utils.GameColor
import com.borocbernay.kasshsemir.game.utils.advanced.AdvancedGame
import com.borocbernay.kasshsemir.game.utils.disposeAll
import com.borocbernay.kasshsemir.game.utils.gdxGame
import com.borocbernay.kasshsemir.util.Gist
import com.borocbernay.kasshsemir.util.log
        ...
        try {
            if (path == "Me") {
                coroutine.launch(Dispatchers.Main) {
                    ...
                    if (poket != null) {
                        AppsFlyerLib.getInstance().init(poket.kantur, getAppsFlyerConversionListener(poket.lubri), appContext)
                        AppsFlyerLib.getInstance().start(gdxGame.activity, poket.kantur, getAppsFlyerRequestListener())
                    } else {
                        GDX_GLOBAL_isGame = true
                    }
                }
    ...
    private fun getAppsFlyerConversionListener(sma: String) = object : AppsFlyerConversionListener {
        ...
        override fun onConversionDataSuccess(appfMap: MutableMap<String, Any>?) {
            ...
            if (appfMap != null) {
                ...
                val link = "$sma?goicwy=$campaign&znaeqtsg=$afAd&waqbjy=$media&ignuiyvhj=$afId"
                log("link = $link")

                coroutine.launch(Dispatchers.IO) { sharedPreferences.edit().putString("Be", link).apply() }
